...
class ProductDiscountEloquentService extends BaseService implements ProductDiscountContract
{
    ...
    public function addDiscountToProduct(DiscountRequest $request, int $id): void
    {
        ...
        if ($group_id) {
            ...
            $discount_id = Discount::create(['product_id' => $id, 'amount' => $data['amount'], 'valid_from' => $data['valid_from'], 'valid_until' => $data['valid_until']])->id;

            DiscountGroup::create(['discount_id' => $discount_id, 'group_id' => $group_id]);
        } else {
            Discount::create(['product_id' => $id, 'amount' => $data['amount'], 'valid_from' => $data['valid_from'], 'valid_until' => $data['valid_until']]);
        }
    }
    ...
    public function updateDiscountFromProduct(DiscountRequest $request, int $id): void
    {
        ...
        if ($group_id) {
            ...
            if ($group) {
                ...
                $discount->update(['product_id' => $id, 'amount' => $data['amount'], 'valid_from' => $data['valid_from'], 'valid_until' => $data['valid_until']]);
            }
            ...
            $discountExcludingGroupDiscount = DB::table('discounts')->whereNotIn('id', $whereNotIn)->get();
            ...
            $finallyDiscount->update(['product_id' => $id, 'amount' => $data['amount'], 'valid_from' => $data['valid_from'], 'valid_until' => $data['valid_until']]);
        }
    }
}
