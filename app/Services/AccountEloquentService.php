...
class AccountEloquentService extends BaseService implements AccountContract
{
    ...
    public function registerAccount(AccountRequest $request): void
    {
        ...
        Account::create(['name' => $request->name, 'email' => $request->email, 'password' => Hash::make($request->password), 'address' => $request->address, 'image_id' => $image_id]);
    }
    ...
    {
        $auth = Account::where([['email', '=', $email], ['email_verified_at', '!=', null]])->first();
