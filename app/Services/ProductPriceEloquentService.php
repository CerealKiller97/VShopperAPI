...
class ProductPriceEloquentService extends BaseService implements ProductPriceContract
{
     ...
     */
    public function addNewPriceToProduct(ProductPriceRequest $request, int $id): void
    {
        ...
        if ($group_id) {
            ...
            if (($group->account_id === null) || ($group->account_id === $account_id)) {
                Price::create(['product_id' => $id, 'amount' => $amount, 'group_id' => $group_id]);
            }
        } else { // price is for everyone not specific price for group
            Price::create(['product_id' => $id, 'amount' => $amount, 'group_id' => null]);
        }
    }
    ...
    public function updatePriceToProduct(ProductPriceRequest $request, int $id): void
    {
        ...
        if ($group_id) {
            ...
            $productPrice = Price::where([['product_id', $id], ['group_id', $group_id]])->latest()->first();

            $productPrice->update(['amount' => $amount, 'product_id' => $id, 'group_id' => $group_id]);
        } else {
            // group_id not passed for all users

            $productPrice = Price::where([['product_id', $id], ['group_id', null]])->latest()->first();

            $productPrice->update(['amount' => $amount, 'product_id' => $id]);
        }
    }
}
